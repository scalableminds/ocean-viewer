...
export class Bridge {
  ...
  private readonly parent: WindowProxy | null = window.parent === window ? null : window.parent;
  ...
  send(message: OutboundMessage): void {
    ...
    const envelope: Envelope<OutboundMessage> = { source: PROTOCOL_SOURCE, ...message };
  ...
  private readonly handleMessage = (event: MessageEvent): void => {
    ...
    if (!this.isOriginAllowed(event.origin)) {
      ...
      console.warn(`[ocean-viewer] rejected message from origin ${event.origin}`);
      return;
    }
