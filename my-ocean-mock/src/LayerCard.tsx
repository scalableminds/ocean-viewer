...
function ColorBar({ layer }: { layer: Layer }) {
	const mid = layer.scale === "log" ? "" : ((layer.min + layer.max) / 2).toString();
	return (
		<div className="colorbar">
			<div className="colorbar-gradient" style={{ background: gradientCss(layer.colormap) }} />
...
export function LayerCard({ layer, expanded, onToggleExpanded, onChange }: Props) {
	return (
		<div className={`layer-card${layer.visible ? "" : " is-hidden"}`}>
			...
			<div className="layer-actions">
				...
				<button type="button" className="action" title="Layer order" tabIndex={-1}>
					{Icon.stack}
				</button>
				<button
					type="button"
				...
				<button
					...
					onClick={() => onChange({ scale: layer.scale === "log" ? "linear" : "log" })}
				>
					log
				</button>
			</div>
